...
public class Handler implements GameInterface
{
     ...
     * Override interface.
     */
    public void collision(GameObject player, GameObject box){
        if(player.getX() >= box.getX()-player.getWidth() && player.getX() <= box.getX() && player.getY() <= box.getY()+player.getHeight() && player.getY() >= box.getY()-player.getHeight()){
            box.setX(rand.nextInt(640));
            box.setY(rand.nextInt(480));
        }
    }
